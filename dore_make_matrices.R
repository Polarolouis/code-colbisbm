...
dore_aggregated_network_matrices <- setNames(lapply(unique(dore_df$id_network_aggreg), function(id) {
    as.matrix(table(dore_df[dore_df$id_network_aggreg == id, c("insectfull", "plantfull")]))
}), nm = aggreg_names)

saveRDS(dore_aggregated_network_matrices, here("data", "dore", "dore_aggregated_networks_matrices.Rds"))
