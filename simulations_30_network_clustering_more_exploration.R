...
stopifnot("Function `clusterize_bipartite_networks_more_exploration` not found" = tryCatch(
    {
        getFromNamespace(x = "clusterize_bipartite_networks_more_exploration", ns = "colSBM")
        TRUE
    },
...
save_filename <- paste0(
    paste0(3 * M, "collection_data_clustering_more_exploration_"),
...
results <- future.apply::future_lapply(
    seq_len(nrow(conditions)), function(s) {
        ...
        list_collection <- clusterize_bipartite_networks_more_exploration(
        ...
        bicl_vec <- sapply(best_partitions, function(col) {
            return(compute_bicl_partition(col$best_fit))
        })
