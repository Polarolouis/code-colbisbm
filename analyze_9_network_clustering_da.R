...
df_desc_asc <- readRDS("simulations/clustering/9collection/9collection_data_clustering_da_15-06-25-14-35-11.Rds")
df_desc_asc$model <- "iid_desc_asc"

df_netclust <- rbind(
    df_netclust,
    df_desc_asc[, c("epsilon", "model", "ARI", "BICL")]
)
...
dev.off()
png(
    file = file.path(output_tikz_folder, "ari-clustering-desc&asc9.png"),
    width = 960,
    height = 960
)
print(ari_plot)
