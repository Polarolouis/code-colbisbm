...
library(patchwork)
plot(fit1$best_fit, type = "meso", mixture = TRUE, values = T)
plot(fit2$best_fit, type = "meso", mixture = TRUE, values = T)
plot(fit3$best_fit, type = "meso", mixture = TRUE, values = T)
```
...
plot(fit123$best_fit, type = "meso", mixture = TRUE, values = T)
```
...
if (!file.exists(clust_path)) {
    ...
    saveRDS(clustering, clust_path)
} else {
    clustering <- readRDS(clust_path)
}
```

```{r}
lapply(clustering$partition, function(col) plot(col$best_fit, type = "meso", mixture = T, values = T))
```

```{r}
lapply(aggr_clustering$partition, function(col) plot(col$best_fit, type = "meso", mixture = T, values = T))
```